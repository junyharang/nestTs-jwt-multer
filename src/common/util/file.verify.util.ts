...
export class FileVerifyUtil {
  public static imageSizeVerify(maxWidth: number, maxHeight: number, imageFile: Express.Multer.File): boolean {
    try {
  ...
  public static async imageResizing(image: Express.Multer.File, maxWidth: number, maxHeight: number): Promise<boolean> {
    let result = false;
    try {
